...
* **wrap (pi):** add session-scoped `headroom wrap pi` provider management with a temporary pi extension, lazy on-demand startup, `/headroom-status`, compact footer/status reporting, and GitHub Copilot dual-wire routing. Copilot now keeps pi's native OAuth flow while routing OpenAI-wire and Anthropic-wire models through separate managed local variants; the default/OpenAI variant starts eagerly while the Anthropic companion variant starts on demand, and surviving wrap-pi sessions can self-heal/take over attached provider ports when the original owner exits. Lazy no-`--provider` sessions now fail open: if a proxy is still starting, the current request can continue on pi's native provider while Headroom finishes background startup and automatically attaches for later requests.
